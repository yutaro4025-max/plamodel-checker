...
:: ── タイムスタンプ取得 ─────────────────────────────────────────
for /f "usebackq tokens=*" %%i in (`powershell -NoProfile -Command "Get-Date -Format 'yyyy-MM-dd HH:mm:ss'"`) do set DT=%%i
...
echo !DT! [INFO] .NET 8 確認中 >> "%LOGFILE%"
...
echo.
echo  手動インストール: https://dotnet.microsoft.com/download/dotnet/8.0
